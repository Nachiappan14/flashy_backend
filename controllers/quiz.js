...
module.exports.getLeaderBoard = async function (req, res) {
  ...
  try {
    ...
    let quizzes = await Quiz.find({ deckId }).populate("userId");

    const map = new Map();
    quizzes.forEach((q) => {
      if (map.get(q.userId.name)) {
        var val = map.get(q.userId.name);
        val.push(q.score);
        map.set(q.userId.name, val);
      } else {
        map.set(q.userId.name, [q.score]);
      }
    });
    const l = [];
    map.forEach((v, k) => {
      l.push({ name: k, avgScore: v.reduce((a, b) => a + b, 0) / v.length });
    });
    l.sort(function (a, b) {
